...
const login = async (_req, _res, _next) => {
  try {
    ...
    if (!user) {
      return _res.status(401).json({ statusCode : 400, message: "Invalid Username or Password" });
    }
    ...
    return _res.status(200).json({message:'user logged in successfully', statusCode: 200, token, user });
  } catch (error) {
